...
package org.osmf.player.chrome.widgets
{
	...
	public class MuteButton extends ButtonWidget
	{
		...
		override protected function onMouseClick(event:MouseEvent):void
		{
			...
			{
				volumeWidget.dispatchEvent(event);
			}
		}
		...
		override protected function onMouseOver(event:MouseEvent):void
		{
			...
			WidgetHint.getInstance(this).widget = volumeWidget;
		...
		protected function onMutedChange(event:AudioEvent = null):void
		{
			if (audible.muted)
			{
				...
				setFace(volumeWidget.slider.sliding ? down : (event ? over : up));
			}
			else
			{
				onVolumeChange();
		...
		protected function onMouseMove(event:MouseEvent):void
		{
			...
			if (event.localY < 0)
			{
				...
				volumeWidget.dispatchEvent(event.clone());  
			}
			else
			{
				volumeWidget.slider.stop();
			}
		}
