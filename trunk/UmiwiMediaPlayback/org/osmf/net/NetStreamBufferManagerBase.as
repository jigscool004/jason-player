            ...
            if(ControlUtil.configuration.isRTMP)
            {
                ...
                switch (event.info.code) 
                {
                    ...
                    case NetStreamCodes.NETSTREAM_BUFFER_EMPTY:					
                        ...
                        bufferComputedTime = computeBufferTime();
                ...
                switch (event.info.code) 
                {
                    case NetStreamCodes.NETSTREAM_PLAY_START:
                        if (!started)
                        {
                            ...
                            netStream.backBufferTime = ControlUtil.configuration.backBufferTime;
                            started = true;	
                            //UConfigurationLoader.updateMsg("Start to buffer. Netstream.inBufferSeek is " + netStream.inBufferSeek);
                        }				
                        break;	
                    case NetStreamCodes.NETSTREAM_SEEK_NOTIFY:
                        // When seeking set a small bufferTime in the beginning to allow the user to see some of the content
                    ...
                    case NetStreamCodes.NETSTREAM_BUFFER_EMPTY:					
                        ...
                        if (bufferComputedTime > 0 && bufferComputedTime != netStream.bufferTime)
                        {
                            ...
                            netStream.bufferTime = bufferComputedTime;		
                            UConfigurationLoader.updateMsg("According download ratio " + metrics.downloadRatio + ", adjust buffer size as: " + bufferComputedTime.toString() + " seconds.");
                        }
                        break;
                }		
            }
